...
TxController.prototype.show = function(req, res) {
  console.log("TxController.show(%s)", req.transaction);
  if (req.transaction) {
...
TxController.prototype.transformTransaction = function(transaction, options, callback) {
  ...
  if(! _.isFunction(callback)) {
    callback({error: 'transformTransaction() callback not a function'}, null);
  }

  console.log("TxController.transformTransaction()");
  console.log("  in transaction " + JSON.stringify(transaction));
  var transformed = {
  ...
  if (!transaction.coinbase) {
    ...
    console.log("  out transformed " + JSON.stringify(transformed));
...
TxController.prototype.send = function(req, res) {
  var self = this;
  console.log("TxController.send()");
  console.log("  rawtx: " + req.body.rawtx);
