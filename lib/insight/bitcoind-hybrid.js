...
// hybrid code between bitcore-node/lib/services/bitcoind.js
// connecting to standard *coind, and the
// custom Iquidus with /ext/listunspent and mongodb indexes.
// Replacement for Insight API wrapper over BitPay custom bitcoind
...
Bitcoin.DEFAULT_CONFIG_SETTINGS = {
  ...
  timestampindex: 0,
  spentindex: 0,
  zmqpubrawtx: null, // tcp://127.0.0.1:28332
  zmqpubhashblock: null, // tcp://127.0.0.1:28332
...
Bitcoin.prototype._initClients = function() {
  ...
  // mock bitcore-node
  this.node = {};
  this.node.stopping = false;
    
  console.log("Bitcoin._initClients(): options " + JSON.stringify(this.options));
  //this.nodes.push(this.options.connect);
    
  Object.defineProperty(this, 'client', {
    get: function() {

	//console.log("self.nodes: " + JSON.stringify(self.nodes));
	console.log("self.nodesIndex: " + self.nodesIndex);
...
Bitcoin.prototype.getAPIMethods = function() {
  var methods = [
    ...
    ['getRawMemPool', this, this.getRawMemPool, 0],
...
Bitcoin.prototype._tryAllClients = function(func, callback) {
  ...
  console.log("Bitcoin._tryAllClients(): nodesIndex: " + nodesIndex);
  console.log("Bitcoin._tryAllClients(): this.nodes.length: " + this.nodes.length);
  var retry = function(done) {
    console.log("Bitcoin._tryAllClients(): callback retry() nodesIndex " + nodesIndex);
...
Bitcoin.prototype._wrapRPCError = function(errObj) {
  console.log("Bitcoin._wrapRPCError(): " + errObj.message);
...
Bitcoin.prototype._initChain = function(callback) {
  var self = this;
  console.log("Bitcoin._initChain(): self.client = " + self.client);
    ...
    self.client.getBlock(response.result, function(err, response) {
      ...
      self.client.getBlockHash(0, function(err, response) {
        ...
        self.getRawBlock(blockhash, function(err, blockBuffer) {
          ...
          console.log('Bitcoin Daemon Ready');
          callback();
        });
      });
...
Bitcoin.prototype._updateTip = function(node, message) {
  ...
  console.log("Bitcoin._updateTip()");
...
Bitcoin.prototype._checkSyncedAndSubscribeZmqEvents = function(node) {
  ...
  function checkAndSubscribe(callback) {
    console.log("Bitcoin._checkSyncedAndSubscribeZmqEvents(): callback checkAndSubscribe()");
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  console.log("Bitcoin._connectProcess(): config " + JSON.stringify(config));
  console.log("Bitcoin._connectProcess(): self.node " + self.node);
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if( config.zmqpubrawtx ) {
      // only connect if zmq set in config
      // only *coind based on Bitcoin v0.12.x and higher have it
      self._initZmqSubSocket(node, config.zmqpubrawtx);
      self._subscribeZmqEvents(node);
    }
    else {
      console.log("ZMQ disabled");
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  console.log("Bitcoin.start(): self.options.servicesConfig.bitcoind.spawn = " + self.options.servicesConfig.bitcoind.spawn);
    console.log("Bitcoin.start(): self.options.servicesConfig.bitcoind.connect = " + JSON.stringify(self.options.servicesConfig.bitcoind.connect));

  async.series([
    function(next) {
      if (self.options.servicesConfig.bitcoind.spawn) {
    ...
    function(next) {
      ...
      if (self.options.servicesConfig.bitcoind.connect) {
        console.log("start(): " + "processing connect []");
        async.map(self.options.servicesConfig.bitcoind.connect, self._connectProcess.bind(self), function(err, nodes) {
          if (err) {
            console.log("start(): " + err.message);
            return callback(err);
          }
          for(var i = 0; i < nodes.length; i++) {
            console.log("start(): " + "self.nodes.push(nodes[" + i + "])");
...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  var cacheKey = self.height + ":" + addresses.join('');
...
Bitcoin.prototype.getAddressUnspentOutputs = function(addressArg, options, callback) {
  var self = this;
  console.log("Bitcoin.getAddressUnspentOutputs() options: ", options);
  var queryMempool = _.isUndefined(options.queryMempool) ? false : options.queryMempool;
  console.log("  queryMempool: ", queryMempool);
  var addresses = self._normalizeAddressArg(addressArg);

  self.getInfo(function(err, info) {
    if (err) return callback(err);

    console.log('getAddressUnspentOutputs(): updated self.height = ' + info.blocks);
    self.height = info.blocks;
  });

  var cacheKey = self.height + ":" + addresses.join('');
  // utxosCache may be missing the latest utxos on chain
  // if the client queried new addr before the tx was confirmed.
  // If there is no ZMQ new block notification hook, we need
  // to add the block height to the cacheKey so there is a new key
  // every new block. 
  console.log("utxosCache cacheKey ", cacheKey);
  ...
  function updateWithMempool(confirmedUtxos, mempoolDeltas) {
    console.log("Bitcoin.getAddressUnspentOutputs() updateWithMempool()");
    /* jshint maxstatements: 20 */
    if (!mempoolDeltas || !mempoolDeltas.length) {
      console.log("  !mempoolDeltas || !mempoolDeltas.length -> returning confirmedUtxos");
  ...
  function finish(mempoolDeltas) {
    console.log("Bitcoin.finish()");

    // wrap in getinfo call to get latest block height -> confirmations
    self.getInfo(function(err, info) {
	if (err) {
	    return callback(err);
	}
	console.log('finish(): updated self.height = ' + info.blocks);
	self.height = info.blocks;
      
        if (utxos) {
	    console.log("  got utxos, updateWithMempool()");
	    return setImmediate(function() {
		callback(null, updateWithMempool(utxos, mempoolDeltas));
	    });
	} else {
	    console.log("  !utxos, getAddressUtxos()");
	    self.getAddressUtxos({addresses: addresses}, function(err, response) {
		if (err) {
		    return callback(self._wrapRPCError(err));
		}
		console.log("response: " + JSON.stringify(response));
		var utxos = response.reverse();
  ...
  if (queryMempool) {
    self._getAddressMempool({addresses: addresses}, function(err, result) {
      if (err) {
	return callback(err);
      }
      finish(result);
...
Bitcoin.prototype._getAddressMempool = function(options, callback) {
  // doesn't exist in *coind except custom BitPay addressindex=1
  // https://bitcore.io/guides/bitcoin/
  // We need to do our own mempool query instead.
  // /api/getrawmempool + getrawtransaction + vout[]
  var self = this;
  var deltas = [];
  ...
  self.getInfo(function(err, info) {
    ...
    console.log('_getAddressMemPool(): updated self.height = ' + info.blocks);
    self.height = info.blocks;
 
    //self._tryAllClients(function(client, done) {
    //console.log("_getAddressMempool() self: ", self);
    self.getRawMemPool({addresses: addresses}, function(err, result) {
      if (err) {
	return callback(self._wrapRPCError(err));
      }
      
      var txids = result;
      console.log("getRawMempool() result txids: ", txids);

      // get results for each txid
      async.each(txids, function(txid, cb) {
	console.log("async.each() txid: ", txid);
	self.getDetailedTransaction(txid, function(err, detailedTx) {
	  console.log("detailedTx: ", detailedTx);

	  // check each vout and locate the change address for this
	  async.each(detailedTx.vout, function(vout, cb2) {
	    console.log("  each vout: ");
	    // if vout.addresses contains the addresses it is change
	    var intersectionAddresses = _.intersection(addresses, vout.addresses);
	    console.log("intersectionAddresses: ", intersectionAddresses);
	    if( intersectionAddresses.length > 0 ) {
	      // deltas is going to have a list of these for the the addresses matched
	      var txdelta = {
		"address": vout.addresses[0],
		"txid": txid,
		"index": vout.n,
		"satoshis": vout.value * 1e8,
		"timestamp": detailedTx.time,
		"prevtxid": detailedTx.vin[0].txid,
		"prevout": detailedTx.vin[0].vout
	      };
	      deltas.push(txdelta);
	    }
	...
	console.log("after async.each(txids)");
...
Bitcoin.prototype.getAddressTxids = function(addressArg, options, callback) {
  ...
  console.log("Bitcoin.getAddressTxids() " + addresses.join(','));
  ...
  function finish() {
    ...
    if (txids && !rangeQuery) {
      ...
      // TODO: replace with mongodb index lookup db.get_address()
      self.getAddressTxs(txidOpts, function(err, txs) {
          if (err) {
              return callback(err);
          }

          // take only txids from array of txs
          //var txids = [];
	  console.log("txs: " + JSON.stringify(txs));

	  async.map(txs, function(x, next) {
		  ...
		  self.txidsCache.set(cacheKey, txids);
              ...
              console.log("allTxids: " + JSON.stringify(allTxids));
	      
	      console.log("Bitcoin.getAddressTxids() finish() end");
...
Bitcoin.prototype._getAddressDetailsForInput = function(input, inputIndex, result, addressStrings) {
  console.log("Bitcoin._getAddressDetailsForInput() vin[" + inputIndex + "]");
  console.log("    in: result.satoshis: " + result.satoshis);
  console.log("    input: " + JSON.stringify(input));
  console.log("    result: " + JSON.stringify(result));
    
  if (!input.address) {
    console.log("Bitcoin._getAddressDetailsForInput(): no input address");
    return;
  }
  ...
  console.log("    out: result.satoshis: " + result.satoshis);
};

Bitcoin.prototype._getAddressDetailsForOutput = function(output, outputIndex, result, addressStrings) {
  console.log("Bitcoin._getAddressDetailsForOutput() vout[" + outputIndex + "]");
  console.log("    in: result.satoshis: " + result.satoshis);
  console.log("    output: " + JSON.stringify(output));
  console.log("    result: " + JSON.stringify(result));

  if (!output.address) {
    console.log("Bitcoin._getAddressDetailsForInput(): no output address");
    return;
  }
...
Bitcoin.prototype._getAddressDetailsForTransaction = function(transaction, addressStrings) {
  ...
  console.log("Bitcoin._getAddressDetailsForTransaction(transaction," + addressStrings + ")");
  ...
  console.log("Bitcoin._getAddressDetailsForTransaction() $.checkState(Number.isFinite(result.satoshis));" + result.satoshis);
...
Bitcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  var self = this;
  console.log("Bitcoin._getAddressDetailedTransaction()");

  self.getDetailedTransaction(
    txid,
    function(err, transaction) {
      if (err) {
	console.log("ERROR: Bitcoin._getAddressDetailedTransaction() " + err.message);
        return next(err);
      }
...
Bitcoin.prototype._paginateTxids = function(fullTxids, fromArg, toArg) {
  console.log("Bitcoin._paginateTxids() ");
  var txids;
  ...
  console.log("Bitcoin._paginateTxids() txids count " + txids.length);
  return txids;
};
...
Bitcoin.prototype.getAddressHistory = function(addressArg, options, callback) {
  ...
  console.log("Bitcoin.getAddressHistory() self.getAddressTxids(()");
  self.getAddressTxids(addresses, options, function(err, txids) {
    ...
    console.log("Bitcoin.getAddressHistory() self.getAddressTxids() callback totalCount " + totalCount);
    try {
    ...
    console.log("Bitcoin.getAddressHistory() async.mapLimit()");
    async.mapLimit(
      txids,
      self.transactionConcurrency,
      function(txid, next) {
	console.log("Bitcoin.getAddressHistory() async.mapLimit() callback");
      ...
      function(err, transactions) {
        ...
        console.log("Bitcoin.getAddressHistory() Success");
        callback(null, {
...
Bitcoin.prototype.sendTransaction = function(tx, options, callback) {
  ...
  console.log("Bitcoin.sendTransaction()");
  ...
  var rpcParams = [tx, allowAbsurdFees];
  // Peercoin only uses 1 parameter
  rpcParams = [tx];
  this.client.sendRawTransaction(rpcParams, function(err, response) {
    if (err) {
      console.log("  err: ", err);
      return callback(self._wrapRPCError(err));
    }
...
Bitcoin.prototype.getRawTransaction = function(txid, callback) {
  var self = this;
  console.log("Bitcoin.getRawTransaction()");
  var tx = self.rawTransactionCache.get(txid);
  if (tx) {
    console.log("  returning tx %s from cache", txid);
    return setImmediate(function() {
      callback(null, tx);
    });
  } else {
	...
	console.log("  returning tx %s from coind rpc", txid);
        done(null, buffer);
      });
    }, callback);
  }
...
Bitcoin.prototype.getTransaction = function(txid, callback) {
  var self = this;
  console.log("Bitcoin.getTransaction()");
...
Bitcoin.prototype.getDetailedTransaction = function(txid, callback) {
  var self = this;
  console.log("Bitcoin.getDetailedTransaction()");
  var tx = self.transactionDetailedCache.get(txid);
  console.log("Bitcoin.getDetailedTransaction() tx " + txid + " from cache: " + JSON.stringify(tx));

    // Note, some values come from Bitcore spentindex=1 extra fields
    // https://bitcore.io/guides/bitcoin/
    
    function addInputsToTx(tx, result, txdb) {
    tx.inputs = [];
    tx.inputSatoshis = 0;
    for(var inputIndex = 0; inputIndex < result.vin.length; inputIndex++) {
      var input = result.vin[inputIndex]; // from rpc
      var inputdb = txdb.vin[inputIndex]; // from mongo

      console.log("    vin[inputIndex]: " + JSON.stringify(input));
      if(txdb && txdb.vin.length === result.vin.length && inputdb.amount) {
        input.valueSat = txdb.vin[inputIndex].amount;
      }
	
      if(!input.valueSat && input.value) {
        input.valueSat = Math.trunc(input.value * 1e8);
      }
      ...
      tx.inputs.push({
        ...
        address: input.address || inputdb.addresses || null,
        satoshis: _.isUndefined(input.valueSat) ? null : input.valueSat
      });
    }
  }

  function addOutputsToTx(tx, result, txdb) {
    tx.outputs = [];
    tx.outputSatoshis = 0;
    for(var outputIndex = 0; outputIndex < result.vout.length; outputIndex++) {
      var out = result.vout[outputIndex];
      var outdb = txdb.vout[outputIndex]; // from mongo

      console.log("    vout[outputIndex]: " + JSON.stringify(out));

      if(!out.valueSat && out.value) {
        out.valueSat = Math.trunc(out.value * 1e8);
      }
  ...
  if (tx) {
    ...
    // get tx from rpc and extra from mongodb
    console.log("Bitcoin.getDetailedTransaction() tx " + txid + " from rpc+mongodb");

    // If we can get tx from RPC, then we can check it immediately
    // after a POST /insight-api/tx/send , which is good.
    // But other requests would be good to check the mongodb,
    // for example, if the tx didn't go thru this node w/o txindex=1.
    
    self._tryAllClients(function(client, done) {
      console.log("-> trying client... ");
      
      // wrap in getinfo call to get latest block height -> confirmations
      console.log("  calling self.getRawTransaction(%s)", txid);
      self.getRawTransaction(txid, 1, function(err, response) {
	//if (err) {
	//  return callback(err);
	//}
        if (err) {
	...
	console.log("rpc getrawtransaction worked");
        var result = response;
        var tx = {
	  hex: result.hex,
	...
	// peercoind getrawtransaction does not have input.value
	// , need to lookup by txid+vin[inputIndex]
	// explorerdb.txes
	db.get_tx(txid, function(txdb){
	  // tx may be in mempool only
	  if(!txdb && tx) {
	    console.log("tx may be in mempool only");
	    //return done(null, tx);
	  }
	  if(!txdb) { return callback({ error: 'tx not found.', hash: txid}) };
	  
	  console.log("  db.get_tx() txdb ",txdb);

	  // missing from RPC
	  if(txdb) {
	    tx.height = txdb.blockindex;
	  }
	  
	  addInputsToTx(tx, result, txdb);
	  addOutputsToTx(tx, result, txdb);
...
Bitcoin.prototype.getInfo = function(callback) {
  console.log("Bitcoin.getInfo() ");

  var self = this;
  this.client.getInfo(function(err, response) {
    if (err) {
      console.log("Bitcoin.getInfo() err: " + JSON.stringify(err));
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
    console.log("Bitcoin.getInfo() result: " + JSON.stringify(result));
    var info = {
      version: result.version,
...
Bitcoin.prototype.getRawMemPool = function(err, callback) {
  console.log("Bitcoin.getRawMemPool() ");

  var self = this;
  this.client.getRawMemPool(function(err, response) {
    if (err) {
      console.log("err: ", err);
      return callback(self._wrapRPCError(err));
    }
...
// new mongodb methods
// https://github.com/bitcoin/bitcoin/pull/10370/commits/65cbc4c31905ae30e5706e859d56427474d500a0
/* 
[{
...
*/
Bitcoin.prototype.getAddressUtxos = function(params, callback) {
    var self = this;
    var addresses = params.addresses;

    console.log("Bitcoin.getAddressUtxos() params: " + JSON.stringify(params));
    
    var utxos = [];
    async.each(addresses, function(addr, cb) {
	console.log("each: " + JSON.stringify(addr));
	db.get_address(addr, function(a){
	    if (a) {
		console.log("a: " + JSON.stringify(a));
		if(a.unspent) {
		    async.each(a.unspent, function(u,cb2) {
			var utxo = {
			    address: a.a_id,
			    txid: u.tx_hash,
			    height: u.height,
			...
			utxos.push(utxo);
			console.log("pushed " + JSON.stringify(utxo));
			cb2();
		    }, function(err) {
	    ...
	    console.log("Bitcoin.getAddressUtxos() cb() utxos: " + JSON.stringify(utxos));
	    cb();
	});
	//if (err) {
	...
	//}
	    ...
	    console.log("Bitcoin.getAddressUtxos() done. callback() utxos: " + JSON.stringify(utxos));
	    callback(null, utxos);
	}
    });
...
Bitcoin.prototype.getAddressTxs = function(params, callback) {
    var self = this;
    var addresses = params.addresses;

    console.log("Bitcoin.getAddressTxs() params: " + JSON.stringify(params));
	...
	db.get_address(addr, function(a){
		...
		async.each(a.unspent, function(u,cb2) {
			...
			outputIndex: u.tx_ouput_n,
			script: u.script,
			satoshis: u.value
		    };
		    txs.push(tx);
		    console.log("pushed " + JSON.stringify(tx));
		    cb2();
		}, function(err) {
	    ...
	    console.log("Bitcoin.getAddressTxs() cb() txs: " + JSON.stringify(txs));
	    cb();
	});
	//if (err) {
	//    return callback(new Error(err));
	//}
	    ...
	    //console.log('All files have been processed successfully');
	    console.log("Bitcoin.getAddressTxs() done. callback() txs: " + JSON.stringify(txs));
	    callback(null, txs);
	}
    });
...
Bitcoin.prototype.getRawTransaction = function(txid, decrypt, callback) {
  console.log("Bitcoin.getRawTransaction() " + txid + ", " + decrypt);

  var self = this;
  this.client.getRawTransaction(txid, decrypt, function(err, response) {
    if (err) {
      console.log("Bitcoin.getRawTransaction() err: " + JSON.stringify(err));
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
    console.log("Bitcoin.getRawTransaction() result: " + JSON.stringify(result));
    var info = result;
    callback(null, info);
  });
};
