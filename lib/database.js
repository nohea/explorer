...
function update_address(hash, txid, n, amount, script, type, cb) {
...
//		console.log("vout:%s,n:%d",txid,n);
            unspent.push({tx_hash:txid,value:amount,tx_pos:n,script:script});
          }
          if (unique == true) {
      ...
      if (type == 'vin') {
        ...
        var newAddress = new Address({
          ...
          unspent: [ {tx_hash:txid,value:amount,tx_pos:n,script:script} ],
          received: amount,
          balance: amount,
        });
      }
          ...
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, txid, nvin[i].n, nvin[i].amount, nvin[i].txid, 'vin', function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].n, vout[t].amount, vout[t].script, 'vout', function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
