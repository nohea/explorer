...
function update_address(hash, txid, n, amount, script, type, blockheight, cb) {
    ...
    if (address) {
      ...
      if ( hash == 'coinbase' ) {
        ...
        lib.is_unique(address.txs, txid, function(unique, index) {
          ...
          var unspent = address.unspent;
          if (type === 'vin') {
	    console.log("vin:%s,n:%d,from:%s",txid,n,script);
            while ((indexrm = unspent.findIndex(function(element,index,array){return element.tx_hash===script && element.tx_pos===n})) !== -1) {
              console.log("deleting unspent: indexrm: ", indexrm);
	      unspent.splice(indexrm,1);
	    }
            sent = sent + amount;
          } else {
            received = received + amount;
	    console.log("vout:%s,n:%d,blockheight:%d unspent.push()",txid,n,blockheight);

	    // check for dup unspent vout in mongo
            while ((indexrm2 = unspent.findIndex(function(element,index,array){return element.tx_hash===txid && element.tx_pos===n})) !== -1) {
              console.log("found existing unspent vout: indexrm2: ", indexrm2);
	    }

	    unspent.push({tx_hash:txid,value:amount,tx_pos:n,script:script,height:blockheight});
          }
          if (unique === true) {
      ...
      if (type == 'vin') {
        ...
        var newAddress = new Address({
          ...
          unspent: [ {tx_hash:txid,value:amount,tx_pos:n,script:script,height:blockheight} ],
          received: amount,
          balance: amount,
        });
      }
          ...
          lib.prepare_vin(tx, function(vin) {
            lib.prepare_vout(tx.vout, txid, vin, function(vout, nvin) {
              lib.syncLoop(vin.length, function (loop) {
                ...
                update_address(nvin[i].addresses, txid, nvin[i].n, nvin[i].amount, nvin[i].txid, 'vin', block.height, function(){
                  loop.next();
                });
              }, function(){
                lib.syncLoop(vout.length, function (subloop) {
                  ...
                  if (vout[t].addresses) {
                    update_address(vout[t].addresses, txid, vout[t].n, vout[t].amount, vout[t].script, 'vout', block.height, function(){
                      subloop.next();
                    });
                  } else {
                    subloop.next();
...
module.exports = {
  ...
  update_richlist: function(list, cb){
    if(list == 'received') {
      Address.find({},{txs:0,unspent:0}).sort({received: 'desc'}).limit(100).exec(function(err, addresses){
      ...
      Address.find({},{txs:0,unspent:0}).sort({balance: 'desc'}).limit(100).exec(function(err, addresses){
        Richlist.update({coin: settings.coin}, {
          balance: addresses,
        }, function() {
          return cb();
  ...
  create_market: function(coin, exchange, market, cb) {
    ...
    newMarkets.save(function(err) {
      if (err) {
        console.log("newMarkets.save: " + err);
        return cb();
      } else {
  ...
  create_richlist: function(coin, cb) {
    ...
    newRichlist.save(function(err) {
      if (err) {
        console.log("newRichlist.save: " + err);
        return cb();
      } else {
